...
- **🕐 Defer & log** — real work, but not urgent and not blocking. Recommend the project's current task tracker and offer to create an item with a draft title + body. Plane has been retired from the development pipeline.
...
If any verdict is genuinely the user's call (e.g. do-now vs defer, or which current task tracker to use), ask via one `AskUserQuestion` batch with a recommended option — then execute the chosen actions (create the issue, run the automatable steps) before ending the turn.
