...
1. **Claim it.** Append this session's id (`printenv CLAUDE_CODE_SESSION_ID`) to `sessions` and set `status: in-progress`. A `feature` epic gets `update_task {fields: {ledger_state: active}}`.
...
5. **Archive.** When the work completes set `status: done` (`abandoned` when Lukáš calls it off) and move the file/directory to `<project-slug>/archive/`; an archived twin of the same slug gets the newer one suffixed `-YYYYMMDD`. Never delete a handoff — say where it went in the hand-back. A `feature` epic gets `next_action` from the hand-back, or `ledger_state: closed` when every gate is done.
